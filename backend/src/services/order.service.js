...
export const orderService = {
    create: async (userId, items) => {
        return await db.transaction(async (tx) => {
            ...
            await tx.insert(orderItems).values(orderItemsToInsert.map(oi => ({ ...oi, orderId: newOrder?.id })));
    ...
    simulateStatusTransitions: async (orderId) => {
        const delays = [10000, 20000, 20000];
        const statuses = [
            'RECEIVED',
            'PREPARING',
            'OUT_FOR_DELIVERY',
